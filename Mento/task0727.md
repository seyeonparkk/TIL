...
function addAsync(a, b) {
  return new Promise((resolve, reject) => {
    // 여기에 두 숫자를 더하고, 결과를 resolve 또는 reject를 사용하여 반환하는 코드를 작성하세요.
    // 힌트: typeof를 사용하여 인자가 숫자인지 확인하세요.
  });
}

// Promise를 사용하는 예제 함수
function calculate() {
  // addAsync 함수를 호출하고, then과 catch 메소드를 사용하여 결과를 출력 및 오류 처리하는 코드를 작성하세요.
}
```
